...
func main() {
	...
	key := flag.String("key", "cert/private.key", "TLS key (requires -cert option)")
	cert := flag.String("cert", "cert/certificate.crt", "TLS certificate (requires -key option)")
	flag.Parse()
	...
	//else {
	//	certFile, keyFile = testdata2.GetCertificatePaths()
	//}
	for _, b := range bs {
